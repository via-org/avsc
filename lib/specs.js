...
function assembleProtocol(fpath, opts, cb) {
  ...
  importFile(fpath, function (err, protocol) {
    ...
    if (schemas) {
      ...
      // Inline any definitions to support circular references.
      protocol.types = inlineDefinitions(schemas);
    }
    cb(null, protocol);
  });
...
/**
 * Modifies (and returns) schemas where type definitions have been inlined. This
 * is needed to support circular references.
 */
function inlineDefinitions(schemas, ns) {
  var named = 1; // Types which can be named.
  var complex = 2; // Types which contain other types.
  var kinds = {
    array: complex,
    enum: named,
    error: named | complex,
    fixed: named,
    map: complex,
    record: named | complex,
  };

  // First, index all named types by name.
  var byName = {};
  var i, l, schema, name;
  for (i = 0, l = schemas.length; i < l; i++) {
    ...
    if (name !== undefined) {
      if (byName[name]) {
        throw new Error(f('duplicate name: %j', name));
      }
      byName[name] = schema;
    }
  }

  // Then iterate over all types in order, expanding as needed.
  var expanded = [];
  var names = Object.keys(byName);
  var inlined = {};
  for (i = 0, l = schemas.length; i < l; i++) {
    schema = schemas[i];
    if (getName(schema) !== undefined) {
      schema = expand(schema, ns);
      if (typeof schema != 'string') { // Already expanded.
  ...
  function getName(schema) {
    if ((kinds[schema.type] & named) && schema.name) {
      return utils.qualify(schema.name, schema.namespace || ns);
    }
  }

  function expand(schema, ns) {
    if (typeof schema == 'string') {
      var derefed = byName[utils.qualify(schema, ns)];
      if (!derefed) {
        return schema; // Primitive types.
      }
      schema = derefed;
    }
    if (!kinds[schema.type]) {
      return schema; // Types which are neither named nor complex.
    }
    if (!(kinds[schema.type] & named)) {
      // Types which can have nested definitions but are not named (array, map).
      if (schema.type === 'array') {
        schema.items = expand(schema.items, ns);
      } else {
        schema.values = expand(schema.values, ns);
      }
      return schema;
    }
    ns = schema.namespace || ns;
    var name = utils.qualify(schema.name, ns);
    if (inlined[name]) {
      return name; // The type has already been inlined.
    }
    inlined[name] = true;
    if (kinds[schema.type] & complex) {
      // Named types which can have nested definitions (error, record).
      var i, l, field;
      for (i = 0, l = schema.fields.length; i < l; i++) {
        field = schema.fields[i];
        field.type = expand(field.type, ns || utils.impliedNamespace(name));
      }
    }
    return schema;
  }
